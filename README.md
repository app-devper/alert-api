...
- `app/core/messaging` — `MessageProvider` interface + SMS (batch + retry) / Web Push (VAPID,
  จัดการ 410 Gone) / LINE (multicast) + dispatcher fan-out ขนานทุกช่องทาง
...
- `GET|POST /public/me[...]` — สถานะ, เช็กเอาต์, push, line, ถอนความยินยอม (X-Session-Token)
...
- `POST /webhook/sms` — delivery report (HMAC signature)
- `GET /health`
